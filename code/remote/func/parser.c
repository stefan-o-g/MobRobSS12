			...
			if (expect(T_COLON)) {
				...
				if (expect(T_BECOMES)) {
					...
					if (token_is(T_NATIVE)) {
						next_token();
						expr = new_funcdec(parent, funcname, paramlist, varlist,
								functions, type, 1, body);
					} else {
						...
						if (token_is(T_CONSTS)) {
							...
							while (token_is(T_COMMA)) {
								next_token();
								last = last->_list.next = new_list(constdec(),
										NULL );
							}
						...
						if (token_is(T_VARS)) {
							...
							if (varlist == NULL ) {
								...
								last = last->_list.next = new_list(vardec(),
										NULL );
							}
						...
						expr = new_funcdec(parent, funcname, paramlist, varlist,
								functions, type, 0, body);

						if (token_is(T_FUNCTION)) {
							struct ast* last = functions = new_list(
									function_dec(expr), NULL );
							while (token_is(T_FUNCTION)) {
								last = last->_list.next = new_list(
										function_dec(expr), NULL );
							}
						}
...
struct ast* equality_expr() {
	...
	while (token_is(T_GT) || token_is(T_GE) || token_is(T_EQ) || token_is(T_NE)
			|| token_is(T_LE) || token_is(T_LT)) {
		enum binop op = (token_is(T_GT) ? GT : token_is(T_GE) ? GE :
							token_is(T_EQ) ? EQ : token_is(T_NE) ? NE :
							token_is(T_LE) ? LE : LT);
		next_token();
...
struct ast* multiplicative_expr() {
	...
	while (token_is(T_MUL) || token_is(T_DIV) || token_is(T_MOD)) {
		enum binop op = (token_is(T_MUL) ? MUL : token_is(T_DIV) ? DIV : MOD);
