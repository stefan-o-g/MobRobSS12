...
\begin{itemize}
	...
	\begin{verbatim}
		...
		7 b
	\end{verbatim}
	Dabei handelt es sich um Tokens (samt zugehöriger Werte), die ein Scanner für die Programmiersprache PL0\textit{*} erzeugt hat. Ein Ziel war es auf den Bot einen PL0-Compiler\textit{*} bzw. Interpreter zu portieren, der im Rahmen der Vorlesung Compilerbau erstellt wurde. Die Trennung erfolgte zwischen Scanner (läuft auf dem PC) und Parser/Codegenerierung/Interpretierung (läuft auf dem Bot), da der lex-basierte Scanner nicht auf den Bot portierbar war und die Trennung an dieser Stelle auch passend war um die kleinst mögliche Menge an Daten übertragen zu müssen. Das angestrebte Ziel war, den Bot über WLAN mit PL0-Code\textit{*} zu steuern. So müsste man den Bot nicht immer neu flashen sondern könnte ihn in PL0\textit{*} programmieren und die Programme einfach per WLAN übertragen.
	Da die erzeugte hex-Datei mit allen Komponenten des Compilers/Interpreters (ohne Scanner) leider zu groß (101kB statt ca. 69kB) für den Bot war und wir ihn nicht flashen konnten wurde an diesem Feature leider nicht weitergearbeitet. Versuche, die hex-Datei klein genug zu bekommen, waren nicht erfolgreich.
	
	\textit{* Hier ist natürlich eine erweiterte Variante von PL0 gemeint.}
	\item get <what>\\
